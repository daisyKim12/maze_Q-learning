...
import math
import time
from my_src.cell import Cell
from my_src.algorithm import depth_first_recursive_backtracker, binary_tree
...
class Maze(object):
    ...
    def __init__(self, num_rows, num_cols, id=0, algorithm = "dfs_backtrack"):
        ...
        self.cost = -1                                                      # added a cost variable to save total cost of the solution
        self.time = -1                                                      # added a cost variable to save total time spent in searching
        self.path = None
        self.initial_grid = self.generate_grid()
        self.grid = self.initial_grid
        self.generate_maze(algorithm, (0, 0))
        self.set_exit_cell_escape()

    def set_exit_cell_escape(self):
        """Sets the is_escape attribute of the cell at the exit coordinate to True."""
        cell = self.grid[self.exit_coor[1]][self.exit_coor[0]]
        cell.is_escape = True

    def is_terminal_coor(self, coor):
        """Check if the given coordinate is a terminal state.

        Args:
            coor (tuple): The coordinate to check.

        Returns:
            bool: True if the coordinate is a terminal state, False otherwise.
        """
        cell = self.grid[coor[1]][coor[0]]
        return cell.is_escape
    
    def is_valid_coor(self, coor):
        """Function that checks if the given coordinates are within the maze bounds.

        Args:
            coor: A tuple of (x, y) coordinates to check

        Return:
            True: If the coordinates are within the maze bounds
            False: If the coordinates are outside the maze bounds
        """
        x, y = coor
    ...
    #     """
    #     Return the reward for a given coordinate.
    #     """
    #     if coor == self.exit_coor:
    #         # positive reward for reaching the exit
    #         return 100
    #     elif coor in self.generation_path:
    #         # negative reward for revisiting a cell already in the path
    #         return -1
    #     else:
    #         # zero reward for all other cells
    #         return 0
        
    def get_reward(self, current_state, next_state, wall_reward):
        x, y = current_state
        neighbours = self.find_neighbours(x, y)  
        validate_neighbours = self.validate_neighbours_solve(neighbours, x, y, self.exit_coor[0], self.exit_coor[1], "brute-force")
        is_wall = False

        if next_state	==	self.exit_coor:
            reward	=	100
        
        elif next_state not in validate_neighbours:
            reward = wall_reward
            is_wall = True

        else:
            reward	=	-0.1
        return reward, is_wall
