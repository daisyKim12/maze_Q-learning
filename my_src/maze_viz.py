...
class Visualizer(object):
    ...
    def show_maze_solution(self):
        ...
        if self.maze.path is None:
            print("Maze path is None, cannot plot solution.")
            return

        list_of_backtrackers = [path_element[0] for path_element in self.maze.path if path_element[1]]

        # Keeps track of how many circles have been drawn
        circle_num = 0

        self.ax.add_patch(plt.Circle(((self.maze.path[0][0][1] + 0.5)*self.cell_size,
                                      (self.maze.path[0][0][0] + 0.5)*self.cell_size), 0.2*self.cell_size,
                                     fc=(0, circle_num/(len(self.maze.path) - 2*len(list_of_backtrackers)),
                                         0), alpha=0.4))

        for i in range(1, self.maze.path.__len__()):
            if self.maze.path[i][0] not in list_of_backtrackers and\
                    self.maze.path[i-1][0] not in list_of_backtrackers:
                circle_num += 1
                self.ax.add_patch(plt.Circle(((self.maze.path[i][0][1] + 0.5)*self.cell_size,
                    (self.maze.path[i][0][0] + 0.5)*self.cell_size), 0.2*self.cell_size,
                    fc = (0, circle_num/(len(self.maze.path) - 2*len(list_of_backtrackers)), 0), alpha = 0.4))
